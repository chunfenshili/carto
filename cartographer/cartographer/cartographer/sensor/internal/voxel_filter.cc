...
std::vector<PointWintIndex> VoxelFilter(
    const std::vector<PointWintIndex>& range_measurements,
    const float resolution) {
  return RandomizedVoxelFilter(range_measurements, resolution,
                               [](const PointWintIndex& range_measurement) {
                                 return range_measurement.point;
                               });
}
