...
void OptimizationProblem2D::Solve(
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (++node_it; node_it != trajectory_end; ++node_it) {
      ...
      // const transform::Rigid3d relative_local_slam_pose1 =
      //     transform::Embed3D(first_node_data.global_match_pose_2d.inverse() *
      //                        second_node_data.global_match_pose_2d);
                             
      // // LOG(INFO)<<transform::Project2D( relative_local_slam_pose1;
      // problem.AddResidualBlock(CreateAutoDiffSpaCostFunction(Constraint::Pose{
      //                              relative_local_slam_pose1,1e6,1e6}),
