...
std::vector<std::pair<float, transform::Rigid2d>>
FastCorrelativeScanMatcher2D::MatchWithTargetsResCure(
    const std::vector<std::pair<float, transform::Rigid2d>>& initial_poses,
    const sensor::PointCloud& point_cloud, int layer,const SearchParameterTest &search_parameter) {
  SearchParameters search_parameters = SearchParameters(
      search_parameter.liner_search_window,
      search_parameter.angle_search_window, point_cloud, limits_.resolution());

  LOG(INFO)<<"layer :"<<layer;

  std::vector<std::pair<float,transform::Rigid2d>> pose_with_score;
  for (auto initial_pose_estimate_withscore : initial_poses) {
    auto initial_pose_estimate = initial_pose_estimate_withscore.second;
    const Eigen::Rotation2Dd initial_rotation =
        initial_pose_estimate.rotation();

    // if (initial_poses.size() == 1) {
    //   LOG(INFO) << "BranchAndBound with Full Layer";
    //   float min_score = 0.2;
    //   const SearchParameters search_parameters(1, 0.5, point_cloud,
    //                                            limits_.resolution());
    //   float score = 0.3;
    ...
    //   LOG(INFO) << "pose_estimate  :" << pose_estimate << " score " << score;
    //   return {std::pair<float, transform::Rigid2d>{score, pose_estimate}};
    // }
    ...
    std::vector<Candidate2D> lowest_resolution_candidates =
        GenerateLowestResolutionCandidates(search_parameters, layer);
    ScoreCandidates(precomputation_grid_stack_->Get(layer), discrete_scans,
                    search_parameters, &lowest_resolution_candidates);



    auto& best_candidate = *lowest_resolution_candidates.begin();
    pose_with_score.push_back(
  ...
  // using PoseSoreType = std::pair<float, transform::Rigid2d>;
  // std::transform(pose_with_score.begin(), pose_with_score.end(),
  //                initial_poses.begin(), pose_with_score.begin(),
  //                [](const PoseSoreType& first,
  //                   const PoseSoreType& secode) -> PoseSoreType {
  //                  return {first.first + secode.first, first.second};
  //                });

  // double sum_sore =
  //     std::accumulate(pose_with_score.begin(), pose_with_score.end(), 0.0,
  //                     [](const double &init, const PoseSoreType& second) -> double {
  //                       return second.first+init;
  //                     });
  // std::transform(pose_with_score.begin(), pose_with_score.end(),
  //                pose_with_score.begin(),
  //                [=](const PoseSoreType& pose_score) -> PoseSoreType {
  //                  return {pose_score.first / sum_sore, pose_score.second};
  //                });

  // //

  //
  std::sort(pose_with_score.begin(), pose_with_score.end(),
            [](const std::pair<float, transform::Rigid2d> lhs,
               const std::pair<float, transform::Rigid2d> rhs) {
 ...
 std::ostringstream info;
  info<< "\n"; 
  for (auto pose : pose_with_score) {
    info << "pose :" << pose.second << " score " << pose.first << "\n";
  }
  if(layer == 1)return pose_with_score; 
  //
  using PoseSoreType = std::pair<float, transform::Rigid2d>;
  std::vector<std::pair<float, transform::Rigid2d>> filter_pose_score;
  filter_pose_score.push_back(*pose_with_score.begin());
  for (const auto& pose : pose_with_score) {
    std::vector<float> distance(filter_pose_score.size(), 0);
    std::transform(
        filter_pose_score.begin(), filter_pose_score.end(), distance.begin(),
        [=](const PoseSoreType& pose_score) -> float {
          return (pose.second.translation() - pose_score.second.translation()).norm();
        });

    std::sort(distance.begin(), distance.end());

    if (*distance.begin() > 2) {
      filter_pose_score.push_back(pose);
    }
  }
  pose_with_score = filter_pose_score;
    info<< "\n";
  for (auto pose : pose_with_score) {
    info << "filter pose : " << pose.second << " score " << pose.first << "\n";
  }
  LOG(INFO)<<info.str();
//
  std::vector<std::pair<float, transform::Rigid2d>> next_initial_poses;
  if (pose_with_score.size() > 3) {
    for (int i = 0; i < (pose_with_score.size() >> 1); i++) {
      next_initial_poses.push_back(pose_with_score[i]);
    }
  } else {
    next_initial_poses = pose_with_score;
  }

//
  std::vector<std::pair<float,transform::Rigid2d>> result;
  auto matche_reuslt =
      MatchWithTargetsResCure(next_initial_poses, point_cloud, layer - 1,
                              {search_parameter.liner_search_window / 2,
                               search_parameter.angle_search_window/2});
  result.insert(result.begin(), matche_reuslt.begin(),matche_reuslt.end());

 return  result;
}

std::vector<std::pair<float, transform::Rigid2d>>
FastCorrelativeScanMatcher2D::MatchWithTargets(
    const std::vector<std::pair<float, transform::Rigid2d>>& initial_poses,
    const sensor::PointCloud& point_cloud, int layer) {
  ...
  for (auto initial_pose_estimate_withscore : initial_poses) {
    ...
    std::vector<Candidate2D> lowest_resolution_candidates =
        GenerateLowestResolutionCandidates(
            search_parameters, precomputation_grid_stack_->max_depth());
    ...
    LOG(INFO) << "BranchAndBound "<<precomputation_grid_stack_->max_depth();
    float min_score = initial_pose_estimate_withscore.first;
    
    const Candidate2D best_candidate = BranchAndBoundIt(
...
if (*distance.begin() > 2) {
  ...
  std::vector<std::pair<float, transform::Rigid2d>> next_initial_poses;
  if (pose_with_score.size() >= 6) {
    next_initial_poses.insert(next_initial_poses.begin(),
                              pose_with_score.begin(),
                              pose_with_score.begin() + 7);
  } else {
    next_initial_poses = pose_with_score;
  }

  //  next_initial_poses = {pose_with_score.begin(),pose_with_score.end()};

  SearchParameterTest search_parameter_test{2,1};
  return MatchWithTargetsResCure(next_initial_poses, point_cloud, 3,
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
    ...
    const sensor::PointCloud& point_cloud, float min_score, float* score,
    transform::Rigid2d* pose_estimate) const {
  // LOG(INFO)<<initial_pose_estimate;
  CHECK(score != nullptr);
...
std::vector<Candidate2D>
FastCorrelativeScanMatcher2D::GenerateLowestResolutionCandidates(
    const SearchParameters& search_parameters,int layer) const {
  const int linear_step_size = 1 << layer;
  int num_candidates = 0;
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
       ++scan_index) {
    const int num_lowest_resolution_linear_x_candidates =
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBoundIt(
    const std::vector<DiscreteScan2D>& discrete_scans,
    const SearchParameters& search_parameters,
    const std::vector<Candidate2D>& candidates, const int candidate_depth,
    float min_score) const {
  ...
  for (const Candidate2D& candidate : candidates) {
    ...
    best_high_resolution_candidate = std::max(
        best_high_resolution_candidate,
        BranchAndBoundIt(discrete_scans, search_parameters,
                       higher_resolution_candidates, candidate_depth - 1,
                       best_high_resolution_candidate.score));
  }
  return best_high_resolution_candidate;
