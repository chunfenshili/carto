...
class AllSubmapPrecomputaitonGridSave;
...
class PrecomputationGrid2D {
 public:
  friend  class ::AllSubmapPrecomputaitonGridSave;
  PrecomputationGrid2D(const Grid2D& grid, const CellLimits& limits, int width,
                       std::vector<float>* reusable_intermediate_grid);
  PrecomputationGrid2D(const Eigen::Array2i offset,
                       const CellLimits& cell_limit, const float min_score,
                       float max_score, const std::vector<uint8> cells)
      : offset_(offset),
        wide_limits_(cell_limit),
...
class PrecomputationGridStack2D {
 public:
  ...
  PrecomputationGridStack2D(const std::vector<PrecomputationGrid2D> precomputation_grids){
...
class FastCorrelativeScanMatcher2D {
 public:
  ...
  FastCorrelativeScanMatcher2D(
      ...
      std::unique_ptr<PrecomputationGridStack2D> precomputation_grid_stack ,
          proto::FastCorrelativeScanMatcherOptions2D& options)
      : options_(options),
        limits_(limits),
        precomputation_grid_stack_(std::move(precomputation_grid_stack)) {}
  ...
  std::vector<std::pair<float, transform::Rigid2d>> MatchWithTargets(
...
struct SearchParameterTest
{
  float liner_search_window;
  float angle_search_window;
};

  std::vector<std::pair<float, transform::Rigid2d>> MatchWithTargetsResCure(
      const std::vector<std::pair<float, transform::Rigid2d>>& initial_poses,
      const sensor::PointCloud& point_cloud, int layer,const SearchParameterTest &search_parameters);
 private:
  ...
  Candidate2D BranchAndBoundIt(const std::vector<DiscreteScan2D>& discrete_scans,
                             const SearchParameters& search_parameters,
                             const std::vector<Candidate2D>& candidates,
                             int candidate_depth, float min_score) const;
