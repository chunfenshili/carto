...
class PoseGraph2D : public PoseGraph {
 public:
  ...
  friend class TrimmingHandle;
  friend class OverlappingSubmapsTrimmer2D;
  PoseGraph2D(
 ...
 private:
  ...
  std::unique_ptr<LocalGloblePoseFusion> local_globle_fusion_;    
  ...
  class TrimmingHandle : public Trimmable {
   public:
    ...
    const std::map<int, PoseGraphInterface::TrajectoryState> GetTrajectoryStates()
        const EXCLUSIVE_LOCKS_REQUIRED(parent_->mutex_);
