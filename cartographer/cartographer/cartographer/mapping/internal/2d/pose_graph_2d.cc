...
NodeId PoseGraph2D::AddNode(
  ...
  // if (data_.trajectories_state.size() >= 1) {
  //   local_globle_fusion_ = std::make_unique<LocalGloblePoseFusion>(
  //       LocalGloblePoseFusionOption{}, this);
  // }
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
  ...
  std::set<NodeId> newly_finished_submap_node_ids;
  {
    ...
    transform::Rigid2d match_globle_pose;
    // if (local_globle_fusion_) {
    //   auto filtered_gravity_aligned_point_cloud =
    //       data_.trajectory_nodes.at(node_id)
    //           .constant_data->filtered_gravity_aligned_point_cloud;
    //   auto init_pose = local_pose_2d;


    //   match_globle_pose = *local_globle_fusion_->Fusion(LocalFusionData{
    //       data_.trajectory_nodes.at(node_id).time(),
    //       std::make_shared<transform::Rigid2d>(init_pose),
    //       std::make_shared<transform::Rigid2d>(match_globle_pose),
    //       filtered_gravity_aligned_point_cloud,
    //   });


    //  auto  match_globle_pose1 = transform::Project2D(
    //       transform::Embed3D(match_globle_pose) *
    //       transform::Rigid3d::Rotation(constant_data->gravity_alignment));
    //  auto  match_globle_pose2 = transform::Project2D(
    //       transform::Embed3D(global_pose_2d) *
    //       transform::Rigid3d::Rotation(constant_data->gravity_alignment));


    //   LOG(INFO)<<match_globle_pose1; 
    //   LOG(INFO)<<match_globle_pose2; 
    // }
    //
    optimization_problem_->AddTrajectoryNode(
        matching_id.trajectory_id,
        optimization::NodeSpec2D{
            constant_data->time, local_pose_2d,global_pose_2d,
            constant_data->gravity_alignment, match_globle_pose});
    ...
    if (newly_finished_submap) {
      ...
      LOG(INFO) << "Newly finished submap.....: " << newly_finished_submap_id.submap_index;
...
PoseGraph2D::TrimmingHandle::GetTrajectoryStates() const {
  std::map<int, PoseGraphInterface::TrajectoryState> trajectories_state;
  for (const auto& it : parent_->data_.trajectories_state) {
    trajectories_state[it.first] = it.second.state;
  }
  return trajectories_state;
}
...
void PoseGraph2D::TrimmingHandle::TrimSubmap(const SubmapId& submap_id) {
  // absl::MutexLock locker(&parent_->mutex_);
