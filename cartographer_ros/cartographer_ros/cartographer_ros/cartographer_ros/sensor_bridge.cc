...
void SensorBridge::HandleLaserScanMessage(
  ...
  // point_cloud = PointCloudProcess(0.4).OutlierFilter(point_cloud);
...
void SensorBridge::HandleRangefinder(
    const std::string& sensor_id, const carto::common::Time time,
    const std::string& frame_id,
    const carto::sensor::PointCloudWithIntensities& ranges) {
  if (!ranges.points.empty()) {
    CHECK_LE(ranges.points.back().time, 0.f);
  }
  const auto sensor_to_tracking =
      tf_bridge_.LookupToTracking(time, CheckNoLeadingSlash(frame_id));
  if (sensor_to_tracking != nullptr) {
    trajectory_builder_->AddSensorData(
        sensor_id, carto::sensor::TimedPointCloudData{
                       time, sensor_to_tracking->translation().cast<float>(),
                       carto::sensor::TransformTimedPointCloud(
                           ranges.points, sensor_to_tracking->cast<float>()),
                       ranges.intensities});
  }
}
}  // namespace cartographer_ros
