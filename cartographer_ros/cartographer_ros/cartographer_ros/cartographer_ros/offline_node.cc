...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory) {
...
#ifdef FLOOR_OPTION
  auto floor_optionzation =
      std::make_unique<cartographer::mapping::FloorPlaneOptimazationWrapper>(
          nullptr);
  cartographer::mapping::scan_matching::SetFloorPlaneOptimazationWrapper(
  ...
  cartographer::mapping::MapById<cartographer::mapping::NodeId, cartographer::mapping::TrajectoryNode>
      trajectory_nodes;
  ...
  if (!FLAGS_load_state_filename.empty()) {
    auto pose_graph = static_cast<cartographer::mapping::PoseGraph*>(
        map_builder_->pose_graph());
   
    // SetLocalGlobleFusion();
  }
  ...
  if (::ros::ok() &&
    ...
    node.SerializeState(state_output_filename,
      ...
      // merge_trajector.Merge(state_output_filename);
  }
