...
class Node {
 public:
  ...
  // Loads a serialized SLAM state from a .pbstream file.
    ...
    void LoadState(
      const std::string& state_filename, bool load_frozen_state,
      cartographer::mapping::MapById<cartographer::mapping::NodeId,
                                     cartographer::mapping::TrajectoryNode>&
          nodes);

  ::ros::NodeHandle* node_handle();
MapBuilderBridge* GetMapBuilderBridge(){ return &map_builder_bridge_;};
 private:
  struct Subscriber {
  ...
  void MapProcess(const ros::WallTimerEvent&);
  std::unique_ptr<SubmapSlicesProcess> submap_slices_process_;
