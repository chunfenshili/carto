...
template <typename T>
std::unique_ptr<std::pair<carto::transform::Rigid3d, carto::common::Time>>
GetTimePose(const T& message, const std::string& tracking_frame,
            const tf2_ros::Buffer& tf_buffer,
            const carto::transform::TransformInterpolationBuffer&
                transform_interpolation_buffer) {
  carto::common::Time start_time = FromRos(message.header.stamp);
  ...
  return std::make_unique<
      std::pair<carto::transform::Rigid3d, carto::common::Time>>(
      std::make_pair<const carto::transform::Rigid3d, carto::common::Time>(
          std::move(tracking_to_map), std::move(start_time)));
  // return nullptr;
}
...
AssetsWriter::AssetsWriter(const std::string& pose_graph_filename,
    ...
    pose_graph_filename_ = pose_graph_filename;
...
void AssetsWriter::Run(const std::string& configuration_directory,
  ...
  do {
    for (size_t trajectory_id = 0; trajectory_id < bag_filenames_.size();
      ...
      // std::string pose_file = pose_graph_filename_ + "_final_pose.txt";
      // std::cout << "Processing pose file...: " << pose_file << std::endl;
      // const carto::transform::TransformInterpolationBuffer transform_interpolation_buffer(pose_file);
      rosbag::Bag bag;
      ...
      for (const rosbag::MessageInstance& message : view) {
        ...
        while (!delayed_messages.empty() && delayed_messages.front().getTime() <
          ...
          if (delayed_message.isType<sensor_msgs::PointCloud2>()) {
            ...
            auto time_pose = GetTimePose(
                *delayed_message.instantiate<sensor_msgs::PointCloud2>(),
                tracking_frame, tf_buffer, transform_interpolation_buffer);
            if(time_pose!=nullptr){
                PosePointWithStamp pose_point;
                auto pose  =  time_pose->first;
                auto timestamp =time_pose->second;
                ...
                pose_point.time = carto::common::ToUniversal(timestamp);
  ...
  if (!pose_point_->empty()) {
    pcl::io::savePCDFileASCII(bag_filenames_[0] + ".pose.pcd", *pose_point_);
  }
}
