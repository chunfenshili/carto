...
Node::Node(
  ...
  thread_= std::thread([this](){
    submap_slices_process_ = std::make_unique<SubmapSlicesProcess>(this);
    while(!kill_){
      MapProcess(ros::WallTimerEvent());
       std::this_thread::sleep_for(std::chrono::milliseconds(3000));
    }
  });

  // submap_slices_process_ = std::make_unique<SubmapSlicesProcess>(this);
  // wall_timers_.push_back(node_handle_.createWallTimer(::ros::WallDuration(1),
  //                                                     &Node::MapProcess, this));
}

bool update  =  false;
void Node::MapProcess(const ros::WallTimerEvent&) {
  static  double rotate = 0;
  static int  num = 0;
  std::string tread_name = "MapProcess";
  ::prctl(PR_SET_NAME, tread_name.c_str());
  // if(update)return;
  update = true; 
  if(!map_builder_bridge_.MapBuilder()){update = false;  return ;}
    auto painted_slices = submap_slices_process_->Process();
    if (!painted_slices.surface) {
      LOG(INFO)<<"painted slices nullptr";
...
void Node::HandleImuMessage(const int trajectory_id,
                            const std::string& sensor_id,
                            const sensor_msgs::Imu::ConstPtr& msg1) {
  // sensor_msgs::Imu::Ptr msg1 = boost::shared_ptr<sensor_msgs::Imu>(*msg);
  sensor_msgs::Imu::Ptr msg = boost::make_shared<sensor_msgs::Imu>(*msg1);
  // msg->header.stamp = msg1->header.stamp + ros::Duration(1640916112.387438);
...
void Node::LoadState(
    ...
    cartographer::mapping::MapById<cartographer::mapping::NodeId,
        ...
        nodes) {
  absl::MutexLock lock(&mutex_);
  map_builder_bridge_.LoadState(state_filename, load_frozen_state, nodes);
}
